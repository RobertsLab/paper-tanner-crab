...
```{r}
uniprot <- read.delim("../data/transcriptome-v-3.1-annot.tab", sep = '\t', header = TRUE)
head(uniprot)
```
12931 annotations 
...
Rename columns:
```{r}
colnames(uniprot) <- c("uniprot_acc_ID", "Entry", "Entry.name", "Status", "Protein.names", "Geme.names", "Organism", "Length")
head(uniprot)
```
