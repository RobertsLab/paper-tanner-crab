...
`join` the `blast` output with the inftemp.clust:  
```{r}
blastinftemp <- left_join(inftemp.clust, blast, by = "Trinity_ID")
head(blastinftemp)
```

`join` the previous file with uniprot-SP-GO:
```{r}
blastinftemp.GO <- left_join(blastinftemp, uniprot_SP_GO, by = "uniprot_acc_ID")
head(blastinftemp.GO)
```

write out table:
```{r}
#write.table(blastinftemp.GO, "../analyses/pheatmap/contrasttemp_DEGs-clusters-annot.tab", sep = '\t', quote = FALSE, row.names = FALSE)
```
