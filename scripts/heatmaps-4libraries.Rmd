...
`join` the `blast` output and uniprot-SP-GO:  
```{r}
blastgo <- left_join(blast, uniprot_SP_GO, by = "uniprot_acc_ID")
head(blastgo)
```

`join` the previous file with inftemp.clust:
```{r}
inftemp.clust.blastgo <- left_join(inftemp.clust, blastgo, by = "Trinity_ID")
head(inftemp.clust.blastgo)
```

write out table:
```{r}
#write.table(inftemp.clust.blastgo, "../analyses/pheatmap/contrasttemp_DEGs-clusters-annot.tab", sep = '\t', quote = FALSE, row.names = FALSE)
```
