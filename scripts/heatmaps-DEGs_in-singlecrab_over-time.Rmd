...
```{r}
singlecrab.genes <- select(singlecrab.genes, "Trinity_ID", "cluster", "V2", "uniprot_acc_ID", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13", "V14", "Entry.name", "Status", "Protein.names", "Geme.names", "Organism", "Length", "Gene.ontology.biological.process", "Gene.ontology.cellular.component", "Gene.ontology.GO", "Gene.ontology.molecular.function", "Gene.ontology.IDs")
head(singlecrab.genes)
```

Rename cluster column and corrent the typo of "Geme.names":
```{r}
colnames(singlecrab.genes) <- c("Trinity_ID", "singlecrab.gene.cluster", "V2", "uniprot_acc_ID", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13", "V14", "Entry.name", "Status", "Protein.names", "Gene.names", "Organism", "Length", "Gene.ontology.biological.process", "Gene.ontology.cellular.component", "Gene.ontology.GO", "Gene.ontology.molecular.function", "Gene.ontology.IDs")
head(singlecrab.genes)
```
...
Rename cluster column in temp.clust
```{r}
colnames(temp.clust) <- c("Trinity_ID","178","359","463", "temp.contrast.clusters")
head(temp.clust)
```


`join` the table of 94 temp-influenced infection DEGs (temp.clust) with the singlecrab.genes:
```{r}
tempcon.singlecrab.genes <- left_join(temp.clust, singlecrab.genes, by = "Trinity_ID")
...
```
Wrote out 09/16/2020. Commented out code. 

Some of the 94 temp-influenced infection DEGs do not show up in the single crab genes over time heatmap clustering. This is due to the fact that I had to pare down the number of genes in order to render the heatmap. The DEGs that are not present (22 of the 94) had too low of counts (less than 63 counts across all time points) and weren't included in the single crab genes over time heatmap. 
