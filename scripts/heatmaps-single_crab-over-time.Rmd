...
### `join` heatmap info with `blast` output from transcriptome v 3.1 and uniprot-SP-GO. save to analyses/heatmaps

Make heatmap matrix into a table with rows names as a column called "Trinity_ID":
```{r}
crab.heatmap2 <- tibble::rownames_to_column(crab.heatmap, "Trinity_ID")
head(crab.heatmap2)
```

`join` with `blast` output:
```{r}
crab.blast <- left_join(crab.heatmap2, blast, by = "Trinity_ID")
head(crab.blast)
```

`join` with uniprot-SP-go:
```{r}
crab.blast.go <- left_join(crab.blast, uniprot_SP_GO, by = "uniprot_acc_ID")
head(crab.blast.go)
```
...
```
Wrote out 08/19/2020. Comment out code. 
